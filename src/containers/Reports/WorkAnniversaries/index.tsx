...
export default function WorkAnniversariesReport() {
    ...
    const [columns, setColumns] = useState<GridColDef[]>([
        ...
        { field: 'length_of_service', headerName: t('employee.job.length_of_service_years'), minWidth: 100, flex: 0.6, type: 'number' },
    ...
    const { data: workAnniversariesReportData = [], isLoading } = useQuery<any>(["work_anniversaries_report"], {
        endpoint: 'reports/anniversary',
        options: { method: "post" },
    }, { enabled: true });
    ...
    const exceljsPreProcess = ({ workbook, worksheet }: GridExceljsProcessInput): any => {
        ...
        worksheet.getCell('A1').value = t('reports.list.work_anniversaries');
    ...
    const onGridStateChange = ({ filter: { filteredRowsLookup }, rows: { idRowsLookup } }: GridState) => {
        if (!isEmpty(filteredRowsLookup)) {
            ...
            let numberOfEmployees = uniqBy(workAnniversariesReportData, 'employee_id').length;
            setNumberOfEmployees(numberOfEmployees)
        }
    };
    ...
    return (
        <>
            <PageHeaderTitle title={t('reports.work_anniversaries', {count: numberOfEmployees, empl: t('employee.employee')})} />
            <div style={{ display: 'flex', height: '85%', margin: '20px 50px', flexDirection: 'column' }}>
                <DataGrid
                    name="anniversaries_report"
                    saveGridState
                    apiRef={apiRef}
                    defaultGroupingExpansionDepth={1}
                    onStateChange={onGridStateChange}
                    loading={isLoading}
                    rows={workAnniversariesReportData}
                    columns={columns}
                    ...
                    excelOptions={{
                        exceljsPreProcess,
                        exceljsPostProcess,
                        fileName: `${format(new Date(), "yyyy-MM-dd'T'HH:mm")} - Profesto - ${t('reports.list.work_anniversaries')}`,
                    }}
                />
            </div>
        </>
